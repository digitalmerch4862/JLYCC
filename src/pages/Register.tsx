...
const Register = () => {
  ...
  const { login } = useAppContext(); // In a real app, this would be a register function
  const navigate = useNavigate();

  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    setLoading(true);
    // Simulate registration by logging in as the visitor mock user
    login('visitor1@gmail.com');
    navigate('/dashboard');
    setLoading(false);
  };
    ...
    <div className="min-h-screen bg-stone-50 dark:bg-stone-900 flex flex-col justify-center py-12 sm:px-6 lg:px-8 font-sans relative">
      ...
      <div className="sm:mx-auto sm:w-full sm:max-w-md">
        ...
        <p className="mt-2 text-center text-sm text-stone-600 dark:text-stone-400">
          ...
          <Link to="/login" className="font-medium text-pink-600 hover:text-pink-500 dark:text-pink-400 dark:hover:text-pink-300 transition-colors">
            Sign in here
          </Link>
        </p>
      </div>
        ...
        <div className="bg-white dark:bg-stone-800 py-8 px-4 shadow-sm sm:rounded-2xl sm:px-10 border border-stone-100 dark:border-stone-700">
          ...
          <div className="mt-6">
            ...
            <div className="mt-6 text-center text-sm text-stone-500 dark:text-stone-400">
              This will log you in as a new visitor to demonstrate the onboarding journey.
            </div>
          </div>
        </div>
      </div>
